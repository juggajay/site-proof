...
**Date:** 26 July 2026 · **Rev 1:** authored 26 Jul (`da847c55`) · **Rev 2:** 26 Jul, incorporating the Opus 5 adversarial review of Rev 1 (verdict **6/10**; 12 blockers `[C1R-B1]`…`[C1R-B12]`, 15 recommendations `[C1R-1]`…`[C1R-15]`, 12 cleared items, D1–D10 verdicts) · **Rev 2.1:** 27 Jul, folding the two **primary-source pack confirmation passes** (§0.1, amendments `[C1C-1]`…`[C1C-8]`) · **Rev 2.2:** 27 Jul, folding **F1 §18** (§0.2, amendments `[C1C-15]`…`[C1C-20]` — test-type canonicalization: attribution is by resolved category, not raw string) · **Rev 2.3:** 27 Jul, folding **D14 §18** (§0.3, amendments `[C1C-10]`…`[C1C-14]` — area is load-bearing for counting, the Section 173 exception is quantified, and Q6's 2-D table needs a new limb) · **Status:** implementation-ready pending no further review objections and the Jay decisions in §16.0.
...
| `[C1C-9]`  | **2026-07-27: `tfnsw-r44.v1` DEREGISTERED from the shipped registry** until the Q6 re-author (D14) — draft evaluation of a confirmed-wrong figure fails the `[C1C-5]` principle. `draft` means plausible-but-unconfirmed, not confirmed-wrong. **Supersedes §7.1's and §11's "ships draft" FOR THIS PACK ONLY.** The pack file and its tests stay: they pin the vocabulary and are the starting point for `tfnsw-q6.v1`. | §7.1, §8.2, §11 C1.1, §15.1 exit item 5, §16 D14                                |
...
against this same file and are **not** applied by this revision. **→ Applied in
Rev 2.3, §0.3.**

### 0.3 Rev 2.3 changelog — the D14 lot-attribute / Q6 fold

**Source:** `docs/plans/d14-q6-pack-spec-2026-07-27.md` (D14, Rev 2) §18, which
files `[C1C-10]` … `[C1C-14]` against this document — the list §0.2 explicitly
left unapplied. D14 resolves this spec's **§16 D14** by recording the two missing
lot attributes: the specified relative compaction band (no new column — it rides
`Lot.testScale`, D14 §3.2) and `Lot.materialType` (one new nullable column, D14
§3.1). With those, `tfnsw-q6.v1` becomes encodable and the Section 173 exception
becomes quantified rather than merely named.

| Tag        | Amendment                                                                                                                                                                                                                                                                                                | Landed in                       |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------- |
| `[C1C-10]` | §3.2.1's bolded "**`Lot.quantityValue` / `quantityUnit` are dead weight for C1 counting**" is **false** as of D14. Area is load-bearing for **every** Q6 count (D14 §5.2) and for Section 173 small-area eligibility (D14 §6.2), on the first two packs the product ships. D5's "honest caveat" goes with it. | §3.2.1, §16 D5                  |
| `[C1C-11]` | The Section 173 gap is **quantified**, not merely named: surface area **< 500 m²**, **3** tests, accepted on the **mean**, threshold raised **+2.0 percentage points**, **Scale A/B only**, and **permissive** (`docs/research/c1-sec173-small-lots-2026-07-27.md` §2(b)).                                  | §4.4, §7.2, §8.2, §16 D14       |
| `[C1C-12]` | §4.4's specimen citation sentence gains the quantified Section 173 exception; D14 §6.2.4 supplies the replacement text.                                                                                                                                                                                   | §4.4                            |
| `[C1C-13]` | §8.2's "how to re-author" claim that Q6's 2-D table is expressible by "`perQuantity` + `minCountByScale`" is **false for four of the five lot-area columns** (D14 §5.2.1's worked counter-example). A new `countByAreaBand` limb is required, specified in D14 §4.3. **§3.2.1 is NOT amended for this tag** — its pairing is correctly scoped to a `> 5,000 m²` lot `[D14R-R5]`. | §8.2                            |
| `[C1C-14]` | §11 C1.2's exit criterion and §12's claim-create row still read **p95 < 2 s**. Superseded by **#1581** (`0d94beba`): the Target-1 budget is **p95 < 3,000 ms at 5,000 members**, accepted by Jay on 2026-07-27.                                                                                            | §11 C1.2, §12                   |

**Not folded here.** D14 §18's two untagged rows — refreshing §3.2/§4.1–§4.3's
proposed-type snippets against the shipped `types.ts` (D14 §19 lists the six
drifts), and the research-corpus corrections — are **not** applied by this
revision. Read D14 §19 before trusting a type snippet in §3.2 or §4.1.
...
**`[C1C-10]` Consequence, restated — the area is LOAD-BEARING, not dead weight.** Rev 2.1 concluded here that "`Lot.quantityValue` / `quantityUnit` are dead weight for C1 counting". That is **false** as of D14: the lot's area selects the Q6 column for **every** count the NSW pack produces (D14 §5.2, `countByAreaBand`) and decides Section 173 small-area eligibility on the VicRoads pack (D14 §6.2) — the first two packs the product ships. They still also serve §3.3's lot-size advisory, and bulk entry (§9.1) is still cheapest built once; those are now secondary reasons, not the only ones. `[C1R-1]`, §16 D5.

The pairing shown above stays correct as written `[D14R-R5]`: it is scoped to a `> 5,000 m²` lot, and on that column `perQuantity` + `minCount` does reproduce Table Q6/L.1. It is the **generalisation** in §8.2 that is wrong — `[C1C-13]`.
...
> **Not ready to conform.** Requires **6** density tests (VicRoads Sec 204, **clause 204.13(a)**, Scale B — **v8.0, November 2025**). **4** verified conforming, **1** pending at lab, **1** without a result. _This lot is under 500 m²: Sec 173 cl. 173.04(d) allows 3 tests instead, accepted on the mean, which must beat the scale requirement by 2.0 %._

**`[C1C-12]` The trailing clause is D14 §6.2.4's replacement specimen**, and it is **conditional**: it appears only when the rule declares a `smallLot` limb **and** the lot is eligible — area resolvable and strictly under 500 m², scale in `smallLot.minCountByScale` (D14 §4.4). It rides the **item text**, not the rule `label`: the label reaches no user, because `shortfallSentence` composes this string from `requiredCount`, the counts and the citation and never reads it (D14 §6.2.5 `[D14X-3]`). A 700 m² lot's sentence must **not** carry it.
...
**`[C1C-11]` Rev 2.3: the exception is now QUANTIFIED, and the label is not where it lands.** `docs/research/c1-sec173-small-lots-2026-07-27.md` §2(b) read cl. 173.04(d) primary-source: surface area **< 500 m²** (strictly less than), **3** tests, accepted on the **mean** of the three, with the threshold raised **+2.0 percentage points**, **Scale A/B only** (Scale C is deliberately absent), and the provision is **permissive** — the contractor may elect it, so it never lowers the required count on its own. D14 §6.2 encodes that as a `smallLot` limb on the VicRoads pack and D14 §6.2.1 ships **eligibility only** first: an advisory item, no count change, no schema change. Two corrections to the paragraph above follow from it — the exception is a set of numbers, not a hedge, and it must be carried in the **item text** (D14 §6.2.5), because the `label` this paragraph relies on reaches no user.
...
- **No small-lot / small-area concept** (Section 173) `[C1C-1]` `[C1C-6]`: the count rule over-states the requirement for a small lot, and the 204.14(c) streak may include lots the specification excludes. Both are stated in the citation text rather than silently approximated. **→ Superseded as a ceiling by `[C1C-11]`:** the exception is **quantified** — **< 500 m²**, **3** tests, on the **mean**, **+2.0 pp**, **Scale A/B only**, **permissive** (`docs/research/c1-sec173-small-lots-2026-07-27.md` §2(b)) — and D14 §6.2 encodes it as a `smallLot` limb. What remains a ceiling is narrower: D14.2 ships **eligibility disclosure only**, so the count is still 6 for an eligible lot until the contractor's election is recordable (D14 §3.3 / §6.2.2, deferred), and CIVOS never checks the **mean** or the **+2.0 pp** threshold — it counts tests (D14 §6.3). The streak exclusion lands with the same deferred phase (D14 §6.4).
...
- **`tfnsw-r44.v1`** — **MISATTRIBUTED. KEEP DRAFT; do NOT confirm as authored; re-author as a Q6 pack** `[C1C-7]`.
  ...
  - **How to re-author.** Identity `tfnsw-q6.v1` (or keep `specSet: 'tfnsw'` with `spec: 'Q6'`), pairing **R44 Table R44.7** _Minimum Characteristic Values of Relative Compaction_ (90.0 / 95.0 / 98.0 / 100.0 / 102.0 % by location — the **acceptance thresholds** that select which Q6 row applies) with **Q6 Table Q6/L.1** _frequencies_. The two are a pair; a correct pack needs both, with R44 Ed 6/Rev 0 recorded as the companion acceptance-criteria source. The rule shape is `(compaction band) × (lot area band) → minCount and/or per-area rate`, which **`perQuantity` + `minCountByScale` already express** (§3.2.1). **→ `[C1C-13]`: that last clause is FALSE and is withdrawn.** The pairing has **one** rate and **one** floor per scale, so it reproduces exactly **one** of the five lot-area columns; it is wrong on the other four. D14 §5.2.1's counter-example: a **400 m²** lot in band `> 95.0–98.0 %` publishes **3**, while the `> 5,000 m²` pairing (`1 per 2,000 m²`, `min 6`) yields **6** — a **100 % over-statement**, and the same pairing under-states elsewhere. The 2-D table needs a **new limb**, `FrequencyRule.countByAreaBand`, specified in **D14 §4.3**; §3.2.1's own example is not affected because it is scoped to a `> 5,000 m²` lot `[D14R-R5]`. The same false claim appears in `docs/research/c1-pack-confirmation-tfnsw-r44-2026-07-27.md` **§4 item 3** and is corrected there by D14 §18.
...
- **Exit:** all three decision points record the block; claim create at the 5,000-member ceiling still meets F0's **p95 < 3,000 ms** `[C1C-14]` (§12 — the 2 s figure Rev 2 carried was superseded by **#1581**, `0d94beba`); `block` proven on a **confirmed** pack and proven inert at `warn`/`off`.
...
| Claim create at the 5,000-member ceiling `[C1C-14]` | **p95 < 3,000 ms unchanged** — the Target-1 budget revised by **#1581** (`0d94beba`, accepted by Jay 2026-07-27), superseding the **2 s** Rev 2 carried; sufficiency adds ≤ 1 grouped query per stream, never per member | F0.5's maximum-size claim benchmark.                                                                                                                                                                                                                                                 |
...
**D5 — Quantity storage.** → **New `Lot.quantityValue`/`quantityUnit`, `LotGeometry.areaM2` as read-time fallback for `m2`, never copied.** Copying stales on the next geometry edit; geometry can never serve volume or tonnage. **The Rev 2.1 "honest caveat" — dead weight for C1 counting — is withdrawn `[C1C-10]`: it is now USED for counts.** Area selects the Q6 column for every NSW count and decides Section 173 eligibility on the VIC pack (§3.2.1, D14 §5.2 / §6.2); §3.3's lot-size advisory and C2/C3 are secondary consumers. The geometry fallback stays read-time and per-unit — D14 §4.6 makes it fire **only** for `m2`, so a lot carrying `4,500 m3` and a polygon no longer resolves to `m3` against an `m2` rule `[D14R-B4]`.
...
**→ RESOLVED by D14 (`docs/plans/d14-q6-pack-spec-2026-07-27.md`, Rev 2) `[C1C-11]`.** Jay took the NSW pack rather than deferring to C2, and the two halves resolved differently: the **compaction band needs no new column** — it rides `Lot.testScale`, whose values are per-ruleset and which Q6 keys to its five bands (D14 §3.2) — while the **material-type half does need one**, `Lot.materialType`, nullable, authority-scoped values (D14 §3.1). The Section 173 row above is likewise superseded: the exception is quantified (**< 500 m²**, **3**, mean, **+2.0 pp**, Scale A/B, permissive) and encoded as a `smallLot` limb (D14 §6.2). The remaining ceilings this row named — no election recorded, no threshold computed — are restated at their true size in §7.2.
