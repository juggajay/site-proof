...
**FURTHER AMENDED 2026-07-29 — the 2,654–2,722 ms figures are still UNVERIFIED in-repo.**
The 2026-07-28 deep review flagged every number in this paragraph as *aspirational*: the
harness was committed and correctly shaped, but no output artefact was, so the figures lived
only in #1641's body. `bench-f05.ts` now **writes a timestamped record on every run,
including runs that error**, so the gap cannot silently reopen. It has **not** been closed:
**four attempts, four failures, zero completed runs.** The 5,000-member decision **exceeds
the 15 s `DECISION_TRANSACTION_TIMEOUT_MS`** and the route 500s — observed transaction
durations **16,815 / 15,149 / 15,299 / 15,469 ms**, i.e. **~5.8× the claimed 2,654 ms**. Two
records are committed, deliberately at the two different surfaces the timeout can surface
at: `f05-2026-07-28T23-05-31-920Z.json` (`SNAPSHOT_WRITE_FAILED` inside
`requirementEvaluation.createManyAndReturn`) and `f05-2026-07-28T23-27-07-094Z.json`
(`DATABASE_ERROR` at `tx.progressClaim.create`, `workflowRoutes.ts:288`).

**Is it the box or the code? Undetermined, and it must stay undetermined until someone runs
this on quiet hardware.** For contention: an unrelated bench (`bench:c3`) slowed by a
comparable factor on the same machine at the same time, and two independent code paths do
not regress by the same multiple. Against a clean attribution: **the box could not be made
idle at all.** The fourth attempt ran with no competing test suite of ours and still
measured `cpuBusyPercent: 100` — the workstation carries a third-party runaway
(`L-Connect 3`, ~43,000 cumulative CPU-seconds) that this session had no business killing.
So "it was contention" is the *likely* reading, not a demonstrated one, and nobody should
record it as demonstrated. **One re-run on genuinely idle hardware is owed before the
2,654 ms figure may be quoted.** The record-writing also means a completed run will finally
ground **AT-13's "178 bytes"** — the harness already measures `snapshotAudit.maxMemberBytes`
against the 1,024-byte budget, and that number, too, is asserted nowhere in code.
