...
describe('Rate Limiting', () => {
  ...
  describe('isLockedOut', () => {
    it('should return false initially for a new IP', async () => {
      ...
      const result = await isLockedOut(testIp);

      expect(result.locked).toBe(false);
      expect(result.remainingSeconds).toBe(0);
    });

    it('should return false after fewer than 5 failed attempts', async () => {
      const testIp = getUniqueIp();

      // Record 4 failed attempts (under threshold)
      for (let i = 0; i < 4; i++) {
        await recordFailedAuthAttempt(testIp);
      }
  ...
  describe('recordFailedAuthAttempt', () => {
    it('should lock out IP after 5 failed attempts', async () => {
      const testIp = getUniqueIp();
  ...
  describe('clearFailedAuthAttempts', () => {
    ...
    it('should allow new attempts after clearing', async () => {
      const testIp = getUniqueIp();
  ...
  describe('lockout timing', () => {
    it('should report remaining seconds when locked out', async () => {
      const testIp = getUniqueIp();
