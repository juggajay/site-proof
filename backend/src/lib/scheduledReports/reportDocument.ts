...
export async function buildScheduledReportDocument(
  ...
  // Scheduled report documents carry the customer's identity like the in-app
  // PDFs do; the text-PDF writer is ASCII-only, so plain text (no logo, no
  // non-ASCII separators).
  const company = await prisma.project
    .findUnique({
      where: { id: schedule.projectId },
      select: { company: { select: { name: true, abn: true } } },
    })
    .then((project) => project?.company ?? null);
