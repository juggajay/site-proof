...
import {
  renderCompanyMemberInvitationEmail,
  type CompanyMemberInvitationEmailData,
} from './email/companyMemberInvitationTemplates.js';
import {
 ...
 *
 * The invite uses the existing reset-password setup page underneath, but the
 * email copy is company-onboarding language so new users are not told they
 * requested a password reset.
 */
export async function sendCompanyMemberInvitationEmail(data: {
  to: string;
  ...
  const { subject, html, text } = renderCompanyMemberInvitationEmail(
    data satisfies CompanyMemberInvitationEmailData,
    {
      escapeEmailHtml,
    },
  );
  ...
  if (process.env.NODE_ENV !== 'production') {
    logInfo('[Email Service] Prepared company member invitation email:', { recipientCount: 1 });
  }
