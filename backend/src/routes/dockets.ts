...
docketsRouter.get('/', asyncHandler(async (req, res) => {
  const user = req.user!
...
docketsRouter.post('/', asyncHandler(async (req, res) => {
  const user = req.user!
...
docketsRouter.get('/:id', asyncHandler(async (req, res) => {
  const { id } = req.params
...
docketsRouter.post('/:id/submit', asyncHandler(async (req, res) => {
  const { id } = req.params
  const user = req.user!
  ...
  if (!hasLabourEntries && !hasPlantEntries) {
    throw new AppError(400, 'At least one labour or plant entry is required before submitting the docket.', 'ENTRY_REQUIRED')
  }
  ...
  if (docket.labourEntries.length > 0) {
    ...
    if (!hasAnyLotAllocation) {
      throw new AppError(400, 'At least one labour entry must be allocated to a lot before submitting the docket.', 'LOT_REQUIRED')
    }
  }
...
docketsRouter.post('/:id/approve', requireRole(DOCKET_APPROVERS), asyncHandler(async (req, res) => {
  const { id } = req.params
  const user = req.user!
...
docketsRouter.post('/:id/reject', requireRole(DOCKET_APPROVERS), asyncHandler(async (req, res) => {
  const { id } = req.params
  const user = req.user!
...
docketsRouter.post('/:id/query', requireRole(DOCKET_APPROVERS), asyncHandler(async (req, res) => {
  const { id } = req.params
  const user = req.user!
  ...
  if (!parseResult.success) {
    throw AppError.badRequest(parseResult.error.errors[0]?.message || 'Questions/issues are required')
  }
...
docketsRouter.post('/:id/respond', asyncHandler(async (req, res) => {
  const { id } = req.params
  const user = req.user!
...
docketsRouter.get('/:id/labour', asyncHandler(async (req, res) => {
  const { id } = req.params
...
docketsRouter.post('/:id/labour', asyncHandler(async (req, res) => {
  const { id } = req.params
  ...
  if (docket.status !== 'draft') {
    throw AppError.badRequest('Can only add labour to draft dockets')
  }
...
docketsRouter.put('/:id/labour/:entryId', asyncHandler(async (req, res) => {
  ...
  if (docket?.status !== 'draft') {
    throw AppError.badRequest('Can only update labour in draft dockets')
  }
...
docketsRouter.delete('/:id/labour/:entryId', asyncHandler(async (req, res) => {
  const { id, entryId } = req.params
  ...
  const docket = await prisma.dailyDocket.findUnique({ where: { id } })
  if (docket?.status !== 'draft') {
    throw AppError.badRequest('Can only delete labour from draft dockets')
  }
...
docketsRouter.get('/:id/plant', asyncHandler(async (req, res) => {
  const { id } = req.params
...
docketsRouter.post('/:id/plant', asyncHandler(async (req, res) => {
  const { id } = req.params
  ...
  if (docket.status !== 'draft') {
    throw AppError.badRequest('Can only add plant to draft dockets')
  }
...
docketsRouter.put('/:id/plant/:entryId', asyncHandler(async (req, res) => {
  const { id, entryId } = req.params
  ...
  const docket = await prisma.dailyDocket.findUnique({ where: { id } })
  if (docket?.status !== 'draft') {
    throw AppError.badRequest('Can only update plant in draft dockets')
  }
...
docketsRouter.delete('/:id/plant/:entryId', asyncHandler(async (req, res) => {
  const { id, entryId } = req.params
  ...
  const docket = await prisma.dailyDocket.findUnique({ where: { id } })
  if (docket?.status !== 'draft') {
    throw AppError.badRequest('Can only delete plant from draft dockets')
  }
