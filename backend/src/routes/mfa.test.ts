...
import { clearFailedAuthAttempts } from '../middleware/rateLimiter.js';
import { enableMfaAndReplaceBackupCodes } from '../lib/mfaBackupCodes.js';
...
describe('MFA API', () => {
  ...
  describe('POST /api/mfa/setup', () => {
    ...
    it('should reject setup when MFA is already enabled', async () => {
      // First enable MFA
      await request(app).post('/api/mfa/setup').set('Authorization', `Bearer ${authToken}`);
  ...
  describe('POST /api/mfa/verify-setup', () => {
    ...
    it('should verify valid code and enable MFA', async () => {
      ...
      expect(res.body.backupCodes.every((code: string) => /^[A-F0-9]{10}$/.test(code))).toBe(true);
    });
    ...
    it('should store backup codes as hashes', async () => {
      const rows = await prisma.$queryRaw<Array<{ code_hash: string; used_at: Date | null }>>`
        SELECT code_hash, used_at FROM mfa_backup_codes WHERE user_id = ${userId}
      `;

      expect(rows).toHaveLength(8);
      expect(rows.every((row) => /^[a-f0-9]{64}$/.test(row.code_hash))).toBe(true);
    ...
    it('should reject malformed setup verification codes', async () => {
      const res = await request(app)
        .post('/api/mfa/verify-setup')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ code: { value: '123456' } });
    ...
    it('should require authentication', async () => {
      const res = await request(app).post('/api/mfa/verify-setup').send({ code: '123456' });
  ...
  describe('POST /api/mfa/disable', () => {
    ...
    it('should normalize backup codes when disabling MFA', async () => {
      const verifyMock = vi.mocked(_otplib.verify);
      ...
      await enableMfaAndReplaceBackupCodes(userId, ['ABCDEF1234']);

      const res = await request(app)
        .post('/api/mfa/disable')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ code: 'abcdef1234' });
  ...
  describe('POST /api/mfa/verify', () => {
    ...
    it('should not consume backup codes through the public verifier', async () => {
      const encryptedSecret = encrypt('TESTSECRET1234567890');
      await prisma.user.update({
        where: { id: userId },
        data: {
  ...
  describe('MFA Full Workflow Integration', () => {
    ...
    it('should prevent double verification during setup', async () => {
      // Setup MFA
      await request(app).post('/api/mfa/setup').set('Authorization', `Bearer ${workflowToken}`);
