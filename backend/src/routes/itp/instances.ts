...
instancesRouter.post('/instances', requireAuth, asyncHandler(async (req: Request, res: Response) => {
  const parseResult = createInstanceSchema.safeParse(req.body)
  if (!parseResult.success) {
    throw AppError.badRequest(parseResult.error.errors[0]?.message || 'lotId and templateId are required')
  }
...
instancesRouter.get('/instances/lot/:lotId', requireAuth, asyncHandler(async (req: Request, res: Response) => {
