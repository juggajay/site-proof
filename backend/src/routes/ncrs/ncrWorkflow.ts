...
ncrWorkflowRouter.post('/:id/respond', requireAuth, asyncHandler(async (req: any, res) => {
...
ncrWorkflowRouter.post('/:id/qm-review', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (!projectUser) {
    throw AppError.forbidden('Only Quality Managers, Project Managers, or Admins can review NCR responses')
  }
...
ncrWorkflowRouter.post('/:id/rectify', requireAuth, asyncHandler(async (req: any, res) => {
...
ncrWorkflowRouter.post('/:id/reject-rectification', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (!projectUser) {
    throw AppError.forbidden('Only Quality Managers, Project Managers, or Admins can reject rectification')
  }
...
ncrWorkflowRouter.post('/:id/qm-approve', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (!isQualityManager) {
    throw AppError.forbidden('Only Quality Managers, Project Managers, or Admins can approve major NCR closures')
  }
...
ncrWorkflowRouter.post('/:id/close', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (ncr.status !== 'verification' && ncr.status !== 'rectification') {
    throw AppError.badRequest('NCR must be in verification or rectification status to close', { currentStatus: ncr.status })
  }

  // CRITICAL: For major NCRs, require QM approval before closing
  if (ncr.severity === 'major' && ncr.qmApprovalRequired && !ncr.qmApprovedAt) {
    throw AppError.forbidden('Major NCRs require Quality Manager approval before closure. Please request QM approval first.')
  }
...
ncrWorkflowRouter.post('/:id/notify-client', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (ncr.clientNotifiedAt) {
    throw AppError.badRequest(`Client was already notified on ${new Date(ncr.clientNotifiedAt).toLocaleDateString()}`)
  }
  ...
  if (!projectUser) {
    throw AppError.forbidden('Only Project Managers, Quality Managers, or Admins can notify client')
  }
...
ncrWorkflowRouter.post('/:id/reopen', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (!projectUser) {
    throw AppError.forbidden('Only Quality Managers can reopen NCRs')
  }
...
ncrWorkflowRouter.post('/:id/submit-for-verification', requireAuth, asyncHandler(async (req: any, res) => {
  ...
  if (ncr.status !== 'rectification' && ncr.status !== 'investigating') {
    throw AppError.badRequest('NCR must be in rectification or investigating status to submit for verification', { currentStatus: ncr.status })
  }

  // Check if evidence has been uploaded
  if (ncr.ncrEvidence.length === 0) {
    throw AppError.badRequest('Please upload at least one piece of evidence before submitting for verification', { evidenceCount: 0 })
  }
