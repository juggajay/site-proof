...
projectsRouter.get('/:id', asyncHandler(async (req, res) => {
  ...
  if (isSubcontractor && subcontractorSuspended) {
    throw AppError.forbidden('Your company has been suspended from this project. Please contact the project manager.')
  }
...
projectsRouter.get('/:id/dashboard', asyncHandler(async (req, res) => {
...
projectsRouter.get('/:id/costs', asyncHandler(async (req, res) => {
  const { id: projectId } = req.params
  const user = req.user!
  ...
  if (user.companyId) {
    ...
    if (company) {
      ...
      if (projectCount >= limit) {
        throw AppError.forbidden(`Your ${tier} subscription allows up to ${limit} projects. Please upgrade to create more projects.`)
      }
    }
  }
...
projectsRouter.get('/:id/users', asyncHandler(async (req, res) => {
  const { id: projectId } = req.params
  const user = req.user!
...
projectsRouter.post('/:id/users', asyncHandler(async (req, res) => {
  const { id: projectId } = req.params
  ...
  if (project?.companyId) {
    ...
    if (company) {
      ...
      if (userCount >= userLimit) {
        throw AppError.forbidden(`Your ${tier} subscription allows up to ${userLimit} users. Upgrade your plan to add more team members.`)
      }
    }
  }
...
projectsRouter.patch('/:id/users/:userId', asyncHandler(async (req, res) => {
...
projectsRouter.delete('/:id/users/:userId', asyncHandler(async (req, res) => {
  const { id: projectId, userId: targetUserId } = req.params
  const currentUser = req.user!
...
projectsRouter.get('/:id/audit-logs', asyncHandler(async (req, res) => {
  const { id: projectId } = req.params
  const user = req.user!
...
projectsRouter.get('/:id/areas', asyncHandler(async (req, res) => {
  const { id: projectId } = req.params
  const user = req.user!
...
projectsRouter.post('/:id/areas', asyncHandler(async (req, res) => {
  const { id: projectId } = req.params
  const user = req.user!
  ...
  if (!name || !name.trim()) {
    throw AppError.badRequest('Area name is required')
  }

  // Feature #906: Require chainage range for areas
  if (chainageStart == null || chainageEnd == null) {
    throw AppError.badRequest('Both chainage start and chainage end are required for project areas.')
  }
  ...
  if (isNaN(startVal) || isNaN(endVal)) {
    throw AppError.badRequest('Chainage values must be valid numbers.')
  }
...
projectsRouter.patch('/:id/areas/:areaId', asyncHandler(async (req, res) => {
...
projectsRouter.delete('/:id/areas/:areaId', asyncHandler(async (req, res) => {
  const { id: projectId, areaId } = req.params
  const user = req.user!
