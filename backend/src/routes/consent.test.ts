...
describe('Consent API', () => {
  ...
  describe('GET /api/consent/types', () => {
    it('should require authentication', async () => {
      const res = await request(app).get('/api/consent/types');
  ...
  describe('GET /api/consent', () => {
    ...
    it('should return the latest consent decision for each type', async () => {
      await request(app)
        .post('/api/consent')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ consentType: 'cookie_policy', granted: true });

      await request(app)
        .post('/api/consent')
        .set('Authorization', `Bearer ${authToken}`)
        .send({ consentType: 'cookie_policy', granted: false });
      ...
      expect(res.status).toBe(200);
      expect(res.body.consents.cookie_policy.granted).toBe(false);
      expect(res.body.consents.cookie_policy.grantedAt).toBeNull();
    });
  });
  ...
  describe('POST /api/consent', () => {
    ...
    it('should reject oversized consent versions', async () => {
      const res = await request(app)
        .post('/api/consent')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
  ...
  describe('POST /api/consent/bulk', () => {
    ...
    it('should reject oversized bulk consent payloads', async () => {
      const res = await request(app)
        .post('/api/consent/bulk')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
  ...
  describe('GET /api/consent/history', () => {
    ...
    it('should reject invalid consent type filter', async () => {
      const res = await request(app)
        .get('/api/consent/history?consentType=invalid_type')
        .set('Authorization', `Bearer ${authToken}`);

      expect(res.status).toBe(400);
      expect(res.body.error.message).toContain('consentType must be one of');
    });
  ...
  describe('POST /api/consent/withdraw-all', () => {
    ...
    it('should require authentication', async () => {
      const res = await request(app).post('/api/consent/withdraw-all');
  ...
  describe('Consent versioning', () => {
    ...
    it('should track multiple versions in history', async () => {
      ...
      await request(app).post('/api/consent').set('Authorization', `Bearer ${authToken}`).send({
        consentType: 'privacy_policy',
        granted: true,
        version: '1.0',
      });
  ...
  describe('User isolation', () => {
    ...
    it('should not see other users consent history', async () => {
      ...
      const hasFirstUserConsent = res.body.history.some((r: any) => r.userId === userId);
  ...
  describe('IP address and user agent tracking', () => {
    ...
    it('should truncate oversized user agents before storing', async () => {
