...
describe('Test Results API', () => {
  ...
  describe('POST /api/test-results/:id/certificate', () => {
    ...
    it('removes the replaced Supabase object after a successful attach', async () => {
      const previousSupabaseUrl = process.env.SUPABASE_URL;
      process.env.SUPABASE_URL = 'https://fixture-project.supabase.co';

      const storagePath = `certificates/${projectId}/cert-old-supabase.pdf`;
      const oldSupabaseUrl = `https://fixture-project.supabase.co/storage/v1/object/public/documents/${storagePath}`;
      ...
      const oldDoc = await prisma.document.create({
        data: {
          ...
          documentType: 'test_certificate',
          category: 'test_results',
          filename: 'old-supabase-cert.pdf',
          fileUrl: oldSupabaseUrl,
          fileSize: 100,
          mimeType: 'application/pdf',
          uploadedById: userId,
        },
      ...
      const testResult = await prisma.testResult.create({
        data: {
          projectId,
          lotId,
          testType: `Supabase Replace Test ${Date.now()}`,
          status: 'entered',
          certificateDocId: oldDoc.id,
          enteredById: userId,
          enteredAt: new Date(),
        },
      });
      ...
      try {
        const res = await request(app)
          ...
          .set('Authorization', `Bearer ${authToken}`)
          .attach('certificate', PDF_BYTES, {
            filename: 'supabase-replacement.pdf',
            contentType: 'application/pdf',
          });

        expect(res.status).toBe(200);
        // New object streamed to Supabase memory storage; replaced one removed.
        ...
        expect(linked.certificateDocId).toBe(res.body.testResult.certificateDoc.id);
        expect(await prisma.document.findUnique({ where: { id: oldDoc.id } })).toBeNull();
      } finally {
        await prisma.testResult.deleteMany({ where: { id: testResult.id } });
        await prisma.document.deleteMany({
          where: {
            projectId,
            filename: { in: ['old-supabase-cert.pdf', 'supabase-replacement.pdf'] },
          },
        });
