...
describe('API Keys Management', () => {
  ...
  describe('POST /api/api-keys', () => {
    ...
    it('should trim API key names and normalize duplicate scopes', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
      ...
      expect(res.status).toBe(201);
      expect(res.body.apiKey.name).toBe('Trimmed API Key');
      expect(res.body.apiKey.scopes).toBe('read,write');
    ...
    it('should reject API key creation with whitespace-only name', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
    ...
    it('should reject API key creation with unsupported scopes', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
          name: 'Unsupported Scope Key',
          scopes: 'read,superuser',
        });

      expect(res.status).toBe(400);
      expect(res.body.error.message).toContain('Invalid API key scopes');
    });

    it('should reject admin-scoped API key creation for non-admin users', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
          name: 'Member Admin Scope Key',
          scopes: 'admin',
        });
    ...
    it('should reject API key creation with zero expiration days', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
    ...
    it('should reject API key creation with negative expiration days', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
    ...
    it('should reject API key creation with expiration beyond one year', async () => {
      const res = await request(app)
        .post('/api/api-keys')
        .set('Authorization', `Bearer ${authToken}`)
        .send({
    ...
    it('should reject non-finite expiration days without creating an API key', async () => {
      const beforeCount = await prisma.apiKey.count({ where: { userId } });
      const body = '{"name":"Infinite Expiry Key","scopes":"read","expiresInDays":1e309}';
      ...
      await expect(prisma.apiKey.count({ where: { userId } })).resolves.toBe(beforeCount);
    });
  });
  ...
  describe('DELETE /api/api-keys/:keyId', () => {
    ...
    it('should reject oversized API key route IDs before lookup', async () => {
      const res = await request(app)
        .delete(`/api/api-keys/${'k'.repeat(121)}`)
        .set('Authorization', `Bearer ${authToken}`);

      expect(res.status).toBe(400);
      expect(res.body.error.message).toContain('keyId is too long');
    });
    ...
    it('should reject revoking without authentication', async () => {
      const res = await request(app).delete(`/api/api-keys/${apiKeyId}`);
  ...
  describe('API Key Authentication Middleware', () => {
    ...
    it('should attach the same user shape as JWT authentication', async () => {
      const testApp = express();
      testApp.use(express.json());
      ...
      expect(res.body.userId).toBe(userId);
      expect(res.body.role).toBe(res.body.roleInCompany);
    ...
    it('should allow API key requests through protected routes that use requireAuth', async () => {
      const testApp = express();
      testApp.use(express.json());

      const { authenticateApiKey } = await import('./apiKeys.js');
      const { requireAuth } = await import('../middleware/authMiddleware.js');
      testApp.get('/protected', authenticateApiKey, requireAuth, (req, res) => {
        res.json({ userId: (req as any).user.id });
      });
      testApp.use(errorHandler);

      const res = await request(testApp).get('/protected').set('x-api-key', generatedApiKey);
    ...
    it('should reject read-scoped API keys on mutating requests', async () => {
      ...
      const { authenticateApiKey } = await import('./apiKeys.js');
      const { requireAuth } = await import('../middleware/authMiddleware.js');
      testApp.post('/protected', authenticateApiKey, requireAuth, (_req, res) => {
        res.json({ success: true });
      });
      testApp.use(errorHandler);

      const res = await request(testApp).post('/protected').set('x-api-key', generatedApiKey);

      expect(res.status).toBe(403);
      expect(res.body.error.message).toContain('Required: write');
    });

    it('should reject API keys for API key management routes', async () => {
      const managementApp = express();
      managementApp.use(express.json());

      const { authenticateApiKey } = await import('./apiKeys.js');
      managementApp.use(authenticateApiKey);
      managementApp.use('/api/api-keys', apiKeysRouter);
      managementApp.use(errorHandler);

      const res = await request(managementApp)
        .get('/api/api-keys')
        .set('x-api-key', generatedApiKey);

      expect(res.status).toBe(403);
      expect(res.body.error.message).toContain('cannot be used to manage API keys');
    });
    ...
    it('should reject requests with invalid API key', async () => {
      ...
      const res = await request(testApp).get('/test').set('x-api-key', 'sp_invalid_key_123');

      expect(res.status).toBe(401);
      expect(res.body.error.message).toContain('Invalid or expired');
    });

    it('should reject empty ApiKey authorization headers', async () => {
      const testApp = express();
      testApp.use(express.json());
      ...
      const res = await request(testApp).get('/test').set('Authorization', 'ApiKey ');
    ...
    it('should reject requests with revoked API key', async () => {
      ...
      const res = await request(testApp).get('/test').set('x-api-key', revokedKey);
    ...
    it('should reject requests with expired API key', async () => {
      ...
      const res = await request(testApp).get('/test').set('x-api-key', 'sp_any_key');
  ...
  describe('Scope Authorization Middleware', () => {
    ...
    it('should allow request with matching scope', async () => {
      ...
      const res = await request(testApp).get('/test').set('x-api-key', readScopeKey);

      expect(res.status).toBe(200);
      expect(res.body.success).toBe(true);
    });

    it('should allow write-scoped API keys on mutating requests', async () => {
      const testApp = express();
      testApp.use(express.json());

      const { authenticateApiKey } = await import('./apiKeys.js');
      const { requireAuth } = await import('../middleware/authMiddleware.js');
      testApp.post('/test', authenticateApiKey, requireAuth, (_req, res) => {
        res.json({ success: true });
      });
      testApp.use(errorHandler);

      const res = await request(testApp).post('/test').set('x-api-key', writeScopeKey);
    ...
    it('should allow admin scope to access any resource', async () => {
      ...
      const res = await request(testApp).get('/test').set('x-api-key', adminScopeKey);
    ...
    it('should allow JWT-authenticated requests to bypass scope checks', async () => {
      ...
      const res = await request(testApp).get('/test').set('Authorization', `Bearer ${authToken}`);
