...
import { sanitizeLogPath, sanitizeLogUrl } from '../lib/logSanitization.js';
...
export function normalizeRequestMetricPath(path: string): string {
...
export function requestLogger(req: Request, res: Response, next: NextFunction) {
  ...
  res.on('finish', () => {
    ...
    const normalizedPath = normalizeRequestMetricPath(req.path);
    ...
    endpointMetrics.avgResponseTime = Math.round(
      (endpointMetrics.avgResponseTime * (endpointMetrics.count - 1) + duration) /
        endpointMetrics.count,
    );
    ...
    // Log through the central sanitizer-backed logger.
    const logLevel =
      res.statusCode >= 400 ? 'error' : duration > SLOW_THRESHOLD_MS ? 'warn' : 'info';
