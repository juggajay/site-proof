...
test.describe('production readiness guardrails', () => {
  ...
  test('backend delay CSV export guards spreadsheet formula prefixes', async () => {
    ...
    expect(diaryReporting).toContain(
  ...
  test('production network calls have bounded timeouts', async () => {
    ...
    const planSheetExtractionSource = await readFile(
      new URL('../../backend/src/routes/copilot/planSheetExtraction.ts', import.meta.url),
      'utf8',
    );
    const lotBreakdownExtractionSource = await readFile(
    ...
    expect(planSheetExtractionSource).toMatch(
      /fetchWithTimeout\(\s*'https:\/\/api\.anthropic\.com\/v1\/messages'/,
    );
    expect(planSheetExtractionSource).toContain('AI_EXTRACTION_TIMEOUT_MS,');
    expect(lotBreakdownExtractionSource).toMatch(
      /fetchWithTimeout\(\s*'https:\/\/api\.anthropic\.com\/v1\/messages'/,
    );
