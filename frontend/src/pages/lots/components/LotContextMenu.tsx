...
export function LotContextMenu({
  ...
  return (
    ...
    >
      ...
      {canCreate &&
        contextMenu.lot.status !== 'conformed' &&
        contextMenu.lot.status !== 'claimed' && (
          <button
            className="w-full px-4 py-2 text-left text-sm hover:bg-muted flex items-center gap-2"
            onClick={() => {
      ...
      <button
        ...
        onClick={() => {
          navigator.clipboard.writeText(
            `${window.location.origin}/projects/${projectId}/lots/${contextMenu.lot.id}`,
          );
          toast({
            title: 'Link Copied',
            description: 'Lot link copied to clipboard',
            variant: 'success',
          });
          onClose();
        }}
