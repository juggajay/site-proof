...
export function useLotsActions({
  ...
  // Quick view state
  const [quickViewLot, setQuickViewLot] = useState<{
    id: string;
  ...
  // Derived
  const deletableLots = displayedLots.filter(
    (lot) => lot.status !== 'conformed' && lot.status !== 'claimed',
  );
  const allDeletableSelected =
    deletableLots.length > 0 && deletableLots.every((lot) => selectedLots.has(lot.id));
  ...
  // Delete handlers
  const handleDeleteClick = useCallback((lot: Lot) => {
    setLotToDelete(lot);
  ...
  // Clone lot
  const handleCloneLot = useCallback(
    async (lot: Lot) => {
      if (cloningLotIdRef.current === lot.id) return;

      cloningLotIdRef.current = lot.id;
      setCloningLotId(lot.id);
      try {
        const data = await apiFetch<{ lot: Lot }>(`/api/lots/${encodeURIComponent(lot.id)}/clone`, {
          method: 'POST',
          body: JSON.stringify({}),
        });
        ...
        cloningLotIdRef.current = null;
        setCloningLotId(null);
      }
    },
    [setLots],
  );
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      const data = await apiFetch<{ message: string }>('/api/lots/bulk-delete', {
        method: 'POST',
        body: JSON.stringify({ lotIds: Array.from(selectedLots) }),
      });
    ...
    async (newStatus: string) => {
      ...
      try {
        const data = await apiFetch<{ message: string }>('/api/lots/bulk-update-status', {
          method: 'POST',
          body: JSON.stringify({ lotIds: Array.from(selectedLots), status: newStatus }),
        });
        setLots((prev) =>
          prev.map((lot) => (selectedLots.has(lot.id) ? { ...lot, status: newStatus } : lot)),
        );
        setSelectedLots(new Set());
            ...
            selectedLots.has(lot.id)
              ? {
                  ...
                  assignedSubcontractor:
                    selectedSubcontractorId && selectedSub
