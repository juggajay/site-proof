...
export const formatTestDate = (value: string | null | undefined): string => {
  if (!value) return '-';
...
export const calculatePassFail = (value: string, min: string, max: string): string => {
  ...
  if (numValue === null) return 'pending';

  const hasMin = min.trim() !== '';
  const hasMax = max.trim() !== '';
  const numMin = hasMin ? parseNumericInput(min) : null;
  const numMax = hasMax ? parseNumericInput(max) : null;

  if ((hasMin && numMin === null) || (hasMax && numMax === null)) return 'pending';
...
export const testTypeSpecs: Record<
  string,
  { min: string; max: string; unit: string; method?: string }
> = {
...
interface BatchConfidenceSource {
  extraction?: {
...
export const getBatchConfidenceIndicator = (
  result: BatchConfidenceSource | null | undefined,
  field: string,
): { color: string; text: string } => {
