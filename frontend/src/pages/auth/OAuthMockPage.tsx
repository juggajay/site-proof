...
export function OAuthMockPage() {
  ...
  const handleMockLogin = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetchWithTimeout(apiUrl('/api/auth/oauth/mock'), {
        method: 'POST',
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <div className="w-full max-w-md space-y-6 rounded-lg bg-card p-8 shadow-lg">
        ...
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Development Mode - Mock OAuth Flow</p>
        </div>
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-warning">
                This is a mock OAuth page for development. In production, you will be redirected to
                Google's actual login page.
              </p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleMockLogin} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-foreground shadow-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
              required
            />
          </div>
